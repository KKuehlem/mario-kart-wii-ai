...
int EnableSkipRendering(lua_State *L)
{
	AI::skipRendering = true;

	return 0; // number of return values
}

int DisableSkipRendering(lua_State *L)
{
	AI::skipRendering = false;

	return 0; // number of return values
}

int GetClientID(lua_State *L)
{
	lua_pushinteger(L, AI::id);

	return 1; // number of return values
}

int Socket_Init(lua_State *L)
{
	AI::socket_init();
...
int Socket_ReadInt(lua_State *L){
	lua_pushinteger(L, AI::SocketReadInt());
	return 1; // number of return values
}
int Socket_ReadDouble(lua_State *L)
{
	lua_pushnumber(L, AI::SocketReadDouble());
	return 1; // number of return values
}
int Socket_ReadString(lua_State *L)
{
	char buffer[1024];
	AI::SocketReadString(buffer, sizeof(buffer));

	lua_pushstring(L, buffer);
	return 1; // number of return values
}

int Socket_SendInt(lua_State *L)
{
	AI::SocketSendInt(lua_tointeger(L, 1));
	return 0; // number of return values
}
int Socket_SendDouble(lua_State *L)
{
	AI::SocketSendDouble(lua_tonumber(L, 1));
	return 0; // number of return values
}
...
namespace Lua
{
	...
	static void RegisterGeneralLuaFunctions(lua_State* luaState)
	{
		...
		lua_register(luaState, "EnableSkipRendering", EnableSkipRendering);
	    lua_register(luaState, "DisableSkipRendering", DisableSkipRendering);
	    lua_register(luaState, "GetClientID", GetClientID);
	    lua_register(luaState, "SocketInit", Socket_Init);
	    lua_register(luaState, "SocketReadInt", Socket_ReadInt);
	    lua_register(luaState, "SocketReadDouble", Socket_ReadDouble);
	    lua_register(luaState, "SocketReadString", Socket_ReadString);
	    lua_register(luaState, "SocketWriteInt", Socket_SendInt);
	    lua_register(luaState, "SocketWriteDouble", Socket_SendDouble);
	}
