...
static inline u32 InterpretDisplayList(u32 address, u32 size)
{
	u8* startAddress;
	...
	if (startAddress != nullptr && !AI::skipRendering)
	{
...
static inline void InterpretDisplayListPreprocess(u32 address, u32 size)
{
...
u8* Run(DataReader src, u32* cycles, bool in_display_list)
{
	...
	while (true)
	{
		...
		case GX_LOAD_CP_REG:
			{
			if (src.size() < 1 + 4) // [No skip possible]
					goto end;
				totalCycles += 12;
		...
		case GX_LOAD_XF_REG:
			{
			if (src.size() < 4) // [No skip possible]
					goto end;
				...
				if (!is_preprocess && !AI::skipRendering)
				{
		...
		case GX_LOAD_INDX_D: //used for lights
			...
			goto load_indx;
		load_indx:
			if (src.size() < 4) // [Cannot skip here]
				goto end;
			totalCycles += 6;
			if (is_preprocess)
		...
		case GX_LOAD_BP_REG:
			...
			{
				if (src.size() < 4) // [Cannot skip all, but might save a bit in methods]
					goto end;
				totalCycles += 12;
				u32 bp_cmd = src.Read<u32>();
				if (is_preprocess || AI::skipRendering) // [This method is simpler]
				{
		...
		if (!is_preprocess && g_bRecordFifoData && cmd_byte != GX_CMD_CALL_DL && !AI::skipRendering)
		{
			u8* opcodeEnd;
