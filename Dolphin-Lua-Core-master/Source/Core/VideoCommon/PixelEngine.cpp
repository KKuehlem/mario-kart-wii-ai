	...
	for (auto& pq_reg : pq_regs)
	{
		mmio->Register(base | pq_reg.addr,
			MMIO::ComplexRead<u16>([pq_reg](u32) {
				return (g_video_backend ? g_video_backend->Video_GetQueryResult(pq_reg.pqtype) : 0) & 0xFFFF;
			}),
			MMIO::InvalidWrite<u16>()
		);
		mmio->Register(base | (pq_reg.addr + 2),
			MMIO::ComplexRead<u16>([pq_reg](u32) {
				return (g_video_backend ? g_video_backend->Video_GetQueryResult(pq_reg.pqtype) : 0) >> 16;
			}),
			MMIO::InvalidWrite<u16>()
		);
	}
	...
	for (int i = 0; i < 4; ++i)
	{
		mmio->Register(base | (PE_BBOX_LEFT + 2 * i),
			MMIO::ComplexRead<u16>([i](u32) {
				...
				return g_video_backend ? g_video_backend->Video_GetBoundingBox(i) : 0;
			}),
			MMIO::InvalidWrite<u16>()
		);
	}
