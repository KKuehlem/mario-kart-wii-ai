...
void DoState(PointerWrap &p)
{
	...
	p.Do(s_skip_current_frame || AI::skipRendering);
...
{
	return s_skip_current_frame || AI::skipRendering;
}
...
static void ReadDataFromFifoOnCPU(u32 readPtr)
{
	...
	s_video_buffer_pp_read_ptr =
	    OpcodeDecoder::Run<true>(DataReader(s_video_buffer_pp_read_ptr, write_ptr + len), nullptr, false);
	...
	s_gpu_mainloop.Run(
	    [] {
		    ...
		    else
		    {
			    ...
			    while (!CommandProcessor::IsInterruptWaiting() && fifo.bFF_GPReadEnable && fifo.CPReadWriteDistance &&
			           !AtBreakpoint())
			    {
				    ...
				    _assert_msg_(COMMANDPROCESSOR, (s32)fifo.CPReadWriteDistance - 32 >= 0,
				                 "Negative fifo.CPReadWriteDistance = %i in FIFO Loop !\nThat can produce instability "
				                 "in the game. Please report it.",
	...
	if (!param.bCPUThread || s_use_deterministic_gpu_thread)
	{
		...
		while (fifo.bFF_GPReadEnable && fifo.CPReadWriteDistance && !AtBreakpoint())
		{
			...
			else
			{
				...
				s_video_buffer_read_ptr =
				    OpcodeDecoder::Run(DataReader(s_video_buffer_read_ptr, s_video_buffer_write_ptr), nullptr, false);
			}
